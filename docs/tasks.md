...
- [x] Section attendance report (`app/(admin)/attendance/reports/section/page.tsx`) — table only, **no trend chart**
- [x] Absenteeism watchlist screen with drill-down (`app/(admin)/attendance/watchlist/page.tsx`) — filterable by term/section, each row links to the student's profile

**Attendance frontend is now complete end-to-end**: take-attendance (teacher route, lock-state + Admin override), the calendar view (Student/Parent/admin profile — one shared component, so all three see identical numbers), section report, absenteeism watchlist, and the excuse-request inbox are all wired to the real backend and verified with a clean build.
- [x] Excuse-request inbox (`app/(teacher)/teacher/excuse-requests/page.tsx`, Teacher approve/reject) — **backend gap found and fixed**: `GET /excuse-requests` didn't exist (same shape as the `GET /student-discounts` gap above), only submit/approve/reject; added it scoped the same way as `assert_can_access_section`, verified by extending the existing excuse-request test (98 tests total — no new test function needed). **Known display gap**: `ExcuseRequestRead` carries no student name (only `attendance_record_id`, and there's no endpoint to resolve one from the frontend), so the inbox shows reason/date/document but not which student by name.
