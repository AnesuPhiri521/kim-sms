...
lives in `app.services.fee_financial`, not here; these are deliberately
thin data-holders.
"""
...
class StudentFeeOverride(Base, AuditMixin):
    """doc 05 §5 — individual per-student amount adjustment
    (e.g. partial-year enrollment).
    """
...
class FeePaymentAllocation(Base, AuditMixin):
    ...
    invoice: Mapped["FeeInvoice"] = relationship(back_populates="allocations", lazy="selectin")

    @property
    def term_id(self) -> str | None:
        """The term of the invoice this slice of the payment settled — read by
        `FeePaymentAllocationRead` so the payment history can show which term(s)
        a payment covered without a second round-trip.
        """

        return self.invoice.term_id if self.invoice is not None else None
...
class Receipt(Base, AuditMixin):
    ...
    payment: Mapped["FeePayment"] = relationship(back_populates="receipt")
...
class FeeLedger(Base, AuditMixin):
    ...
    student_id: Mapped[str] = mapped_column(String(36), ForeignKey("students.id"), index=True)
    # charge|payment|refund|adjustment|credit_issued|credit_applied|credit_refunded
